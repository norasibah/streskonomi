document.addEventListener("DOMContentLoaded", () => {
  ...
  // We will dynamically query elements in update functions to support both table rows and accordion items
  // const factorCards = ... (removed static initialization)
  ...
  const OVERALL_RECOMMENDATIONS = {
    ...
    Tinggi: {
      ...
      interpretation:
        "Streskonomi tahap TINGGI; berisiko beri kesan negatif pada kesihatan mental, hubungan sosial dan prestasi kerja.",
      actions: [
        ...
        "Dapatkan nasihat professional dalam hal pengurusan kewangan - Contoh: Agensi Kaunseling dan Pengurusan Kredit (AKPK), My Personal Finances (MyPF), dan Financial Markets Ombudsman Service (FMOS).",
        "Dapatkan nasihat professional dalam hal pengurusan stress - Contoh: Hotline nasional HEAL Line 15555; Malaysian Mental Health Association (MMHA), Befrienders, LifeLine Association Malaysia.",
      ],
    },
    "Sangat Tinggi": {
      icon: "🔴",
      ...
      interpretation:
        "Streskonomi tahap SANGAT TINGGI; berisiko tinggi mendapat masalah mental tahap 2 iaitu masalah kebimbangan (Anxiety) jika tidak dirawat.",
      actions: [
  ...
  function resetResults() {
    ...
    // Reset all factor displays (both table rows and accordion items)
    document.querySelectorAll("[data-result]").forEach((container) => {
      const mean = container.querySelector(".factor-mean");
      const level = container.querySelector(".factor-level");
  ...
  function updateFactorCard(key, mean, level) {
    // Find all elements for this key (desktop table row AND mobile accordion item)
    const containers = document.querySelectorAll(`[data-result="${key}"]`);

    containers.forEach((container) => {
      const meanEl = container.querySelector(".factor-mean");
      const levelEl = container.querySelector(".factor-level");
  ...
  accordionHeaders.forEach((header) => {
    header.addEventListener("click", () => {
      ...
      // Close all other items (optional: for "one open at a time" behavior)
